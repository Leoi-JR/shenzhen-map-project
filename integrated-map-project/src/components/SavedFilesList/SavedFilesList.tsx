...
const SavedFilesList: React.FC<SavedFilesListProps> = ({ visible = true }) => {
  ...
  const fetchSavedFiles = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://106.13.45.251:5200/api/saved-files');
  ...
  const handleDownload = async (filename: string) => {
    try {
      const response = await fetch(`http://106.13.45.251:5200/api/download/${filename}`);
  ...
  const handleDelete = async (filename: string) => {
    try {
      const response = await fetch(`http://106.13.45.251:5200/api/delete/${filename}`, {
        method: 'DELETE'
      });
